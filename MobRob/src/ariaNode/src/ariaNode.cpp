...
docker build -t ros2-workspace .
*   To run docker containers:
docker run --rm -it --device /dev/input/js0 --device /dev/ttyUSB0 --privileged --env DISPLAY=:1 --volume /tmp/.X11-unix:/tmp/.X11-unix --device=/dev/bus/usb:/dev/bus/usb --network host --shm-size=1g ros2-workspace
*   To run controller nodes use:
ros2 launch controller my_script_launch.py
*   To run this code (to connect to motors) use:
ros2 run ariaNode ariaNode -rp /dev/ttyUSB0
*   Author: Kieran Quirke-Brown
*   Date: 12/01/2024
to run the camera container:
docker run --rm -it --device /dev/input/js0 --device /dev/ttyUSB0 --privileged --env DISPLAY=:1 --volume /tmp/.X11-unix:/tmp/.X11-unix --device=/dev/bus/usb:/dev/bus/usb ros2-workspace
docker run --rm  -it   --privileged     -v /dev/bus/usb:/dev/bus/usb     --device-cgroup-rule='c 189:* rmw'     -e DISPLAY=:1     -v /tmp/.X11-unix:/tmp/.X11-unix     ros2-workspace
ros2 launch depthai_ros_driver camera.launch.py
ensure that "xhost +local:" has been run on the host machine to allow access to the display
*/
